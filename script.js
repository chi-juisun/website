...
const scenarios = [
  ...
  },
  {
    ...
    scenario: ["教職員辦公室外"],
    narration: [],
    dialogue: ["我: 有人嗎", "非: 進去看看吧"],
  },
  {
    //3
    scenario: ["職員辦公室內"],
    narration: [],
    dialogue: ["芷雅: 握草這邊發生甚麼事了", "我: 小心"],
  },
  {
    //4
    scenario: [],
    narration: ["此時受傷的教職員撲了過來 並發動攻擊"],
    dialogue: ["非: 該做點甚麼吧", "我: 讓我想想", "我: 或許我們能"],
    options: [
  ...
  {
    //7
    dialogue: ["我:交給我 我來試著安撫她", "教職員:(拿起桌邊的剪刀)"],
    scenario: [
      "視野黑了一半 一切似乎還行",
      "此時教職員更用力的插入剪刀  你的腦-----",
      "此時的你倒在地上  ---流",
...
function renderScenario(index) {
  ...
  if (
    Array.isArray(currentScenario.scenario) &&
    currentScenario.scenario.length > 0
  ) {
...
scenarioElement.addEventListener("click", () => {
  ...
  if (dialogIndex < dialogue.length - 1) {
    renderDialogue(dialogue, dialogIndex + 1, options);
  } else {
    currentIndex += 1;
...
function moveToNextStep(step) {
  ...
  console.log(scenarios[currentIndex]);
  ...
  switch (step) {
    ...
    case "narration":
      ...
      break;
    case "dialogue":
      console.log(currentScenario.options);
